...
class DoenetBranchBrowser extends Component {
    ...
    directoryList.push(
      <DropItem 
        ...
        <div 
          ...
          <label>
          {console.log("Course code error all course info", this.props.selectedCourse, this.props.allCourseInfo)}
  ...
  buildFolderItems() {
    ...
    console.log("all folder info child folder error", this.props.allFolderInfo)

    // show items in current directory
    if (this.props.directoryStack.length !== 0) {
      let folderId = this.peekDirectoryStack();
      console.log("all folder info child folder Id", folderId)
