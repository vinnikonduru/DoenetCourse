...
const queryCache = new QueryCache(); // >>> QueryCache renamed to QueryClient
...
return <>
  <DropTargetsProvider>
    ...
    {/* // >>> ReactQueryCacheProvider to QueryClientProvider*/}
      <Tool />
      <ReactQueryDevtools />
    </ReactQueryCacheProvider>
  </DropTargetsProvider>
...
function Tool(props){
  ...
  const cache = useQueryCache(); // >>> useQueryCache has changed to useQueryClient
  ...
  const [moveNodes] = useMutation(mutationMoveNodes, { // >>> its now not an array, should be changed to obj
    onMutate:(params)=>{
...
function AddItem(props){
  ...
  function AddItemButton(props){
    const cache = useQueryCache(); // >>> useQueryCache renamed to useQueryClient
...
function BrowserChild(props){
  ...
  const {
    ...
    isFetchingMore, 
    fetchMore, 
    error} = useInfiniteQuery(['nodes',props.drive], fetchChildrenNodes, { // >>> useInfiniteQuery is bi-directional & fetchChildrenNodes fn doennot take any input
      refetchOnWindowFocus: false,
      refetchOnMount:false,
      staleTime:600000,
      onSuccess: (data) => {
        // >>> data should be accessed in a different way... there will be one more option pages in side data orf traversing
        if (Object.keys(data[0])[0] === "init"){
          let folderChildrenIds = {};
          let nodeObjs = {};
          
          for (let row of data[0].data.results){ /// >>> data is now obj containing pages
      ...
      getFetchMore: (lastGroup, allGroups) => { // >>> getFetchMore is renamed to getNextPageParam
